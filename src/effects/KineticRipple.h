...
namespace KineticRipple {
    ...
    inline void update(PixelStrip::Segment* seg) {
        ...
        if (rippleActive) {
            ...
            int b255 = 255 - (radius * 255 / halfLen);
            b255 = constrain(b255, 0, 255);

            RgbColor fcol = rippleColor;
            fcol.Dim(b255);
            fcol.Dim(seg->getBrightness());

            int hw = seg->rippleWidth / 2;
            bool drawn = false;

            for (int i = 0; i < seg->rippleWidth; ++i) {
                int p1 = center - radius - hw + i;
                if (p1 >= s && p1 <= e) {
                    seg->getParent().getStrip().SetPixelColor(p1, fcol);
                    drawn = true;
                }
            }

            int p2 = center + radius - hw;
            if (p2 != center) {
                for (int i = 0; i < seg->rippleWidth; ++i) {
                    int p = p2 + i;
                    if (p >= s && p <= e) {
                        seg->getParent().getStrip().SetPixelColor(p, fcol);
                        drawn = true;
                    }
                }
            }

            if (!drawn && elapsed > 100) {
                rippleActive = false;
            }
        }
    }
