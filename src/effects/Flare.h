...
namespace Flare
{
    ...
    inline void update(PixelStrip::Segment *seg)
    {
        ...
        byte chance = seg->triggerIsActive ? map(seg->triggerBrightness, 0, 255, 150, 255) : seg->fireSparking;
        if (random(255) < chance)
            flare_heat[s + random(7)] = qadd8(flare_heat[s + random(7)], random(160, 255));
        for (int j = s; j <= e; ++j)
        {
