...
#include "BaseEffect.h" // <-- Make sure this is in your include path!
#include <Arduino.h>

class ColoredFireEffect : public BaseEffect {
private:
    PixelStrip::Segment* segment;
    EffectParameter params[5];
    byte heat[300]; // Max 300 LEDs per segment
...
public:
    ColoredFireEffect(PixelStrip::Segment* seg) : segment(seg) {
        // Parameter 1: Sparking
        ...
        // Parameter 3: Color 1 (base)
        params[2].name = "color1";
        params[2].type = ParamType::COLOR;
        params[2].value.colorValue = 0x000000; // Black
        // Parameter 4: Color 2 (middle)
        params[3].name = "color2";
        params[3].type = ParamType::COLOR;
        params[3].value.colorValue = 0xFF0000; // Red
        // Parameter 5: Color 3 (tip)
        params[4].name = "color3";
        params[4].type = ParamType::COLOR;
        params[4].value.colorValue = 0xFFFF00; // Yellow
    ...
    // Optionally, implement setParameter by name/type
