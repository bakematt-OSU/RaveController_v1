...
void BinaryCommandHandler::handleCommand(const uint8_t *data, size_t len) {
    if (len == 1) { // Handle simple, single-byte commands for quick actions
        BleCommand cmd = (BleCommand)data[0];
        switch (cmd) {
            case CMD_GET_LED_COUNT: handleGetLedCount(); break;
            case CMD_GET_ALL_EFFECTS: handleGetAllEffectsCommand(false); break;
            case CMD_GET_ALL_SEGMENT_CONFIGS: handleGetAllSegmentConfigs(false); break;
            case CMD_SAVE_CONFIG: handleSaveConfig(); break;
            case CMD_CLEAR_SEGMENTS: handleClearSegments(); break;
            default: Serial.println("ERR: Unknown single-byte command.");
        }
        return;
    }

    if (len < 2) { Serial.println("ERR: Runt packet received."); return; }
    ...
    if (sequence != _expectedIncomingSequence) { return; }
    ...
    if (_jsonBufferIndex + payloadLen >= sizeof(_incomingJsonBuffer)) {
        ...
        _jsonBufferIndex = 0; // Reset the buffer index
        memset(_incomingJsonBuffer, 0, sizeof(_incomingJsonBuffer)); // Clear the buffer
        // Optionally, send an error message back to the app here
        return; 
    }
...
void BinaryCommandHandler::update() {
    ...
    if (!_isWaitingForAck && !_outgoingPacketQueue.empty()) {
        ...
        // --- FIX: Use std::vector to avoid VLA warning ---
...
void BinaryCommandHandler::processSingleSegmentJson(const char *jsonString) {
    handleBatchConfigJson(jsonString); // Delegate to the main config handler
}
...
String BinaryCommandHandler::buildEffectInfoJson(uint8_t effectIndex) {
    ...
    if (!tempEffect) { delete dummySegment; return "{}"; }
    ...
    for (int i = 0; i < tempEffect->getParameterCount(); ++i) {
        ...
        p_obj["name"] = p->name;
        switch (p->type) {
            case ParamType::INTEGER: p_obj["type"] = "integer"; p_obj["value"] = p->value.intValue; break;
            case ParamType::FLOAT:   p_obj["type"] = "float";   p_obj["value"] = p->value.floatValue; break;
            case ParamType::COLOR:   p_obj["type"] = "color";   p_obj["value"] = p->value.colorValue; break;
            case ParamType::BOOLEAN: p_obj["type"] = "boolean"; p_obj["value"] = p->value.boolValue; break;
        }
...
String BinaryCommandHandler::buildSegmentInfoJson(uint8_t segmentIndex) {
    ...
    if (s->activeEffect) {
        ...
        for (int i = 0; i < s->activeEffect->getParameterCount(); ++i) {
            ...
            switch (p->type) {
                case ParamType::INTEGER: doc[p->name] = p->value.intValue; break;
                case ParamType::FLOAT:   doc[p->name] = p->value.floatValue; break;
                case ParamType::COLOR:   doc[p->name] = p->value.colorValue; break;
                case ParamType::BOOLEAN: doc[p->name] = p->value.boolValue; break;
            }
        }
    } else { doc["effect"] = "None"; }
    String response;
    serializeJson(doc, response);
    return response;
}
...
// In src/BinaryCommandHandler.cpp
