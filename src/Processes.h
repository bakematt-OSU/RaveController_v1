...
inline void handleCommandLine(const String &line)
{
    ...
    else if (cmd == "listsegments")
    {
        ...
        {
            auto *s = v[i];
            Serial.print("["); Serial.print((unsigned)i); Serial.print("] ");
            Serial.print(s->getName().c_str());
            Serial.print(": "); Serial.print(s->startIndex());
            Serial.print("-"); Serial.print(s->endIndex());
            Serial.print(", Brightness="); Serial.println(s->getBrightness());
        }
    }
    ...
    else if (cmd == "getstatus")
    {
        ...
        for (size_t i = 0; i < v.size(); ++i)
        {
            ...
            Serial.print(",\"name\":\""); Serial.print(s->getName().c_str()); Serial.print("\"");
    ...
    // --- New: Effect parameter definitions ---
    else if (cmd == "geteffectinfo")
    {
        StaticJsonDocument<2048> doc;
        JsonArray effectsArr = doc.createNestedArray("effects");
        auto &segments = strip.getSegments();

        for (uint8_t i = 0; i < EFFECT_COUNT; ++i)
        {
            JsonObject eff = effectsArr.createNestedObject();
            eff["name"] = EFFECT_NAMES[i];
            JsonArray params = eff.createNestedArray("params");

            switch ((PixelStrip::Segment::SegmentEffect)i)
            {
                case PixelStrip::Segment::SegmentEffect::Fire:
                {
                    JsonObject p1 = params.createNestedObject();
                    p1["name"]    = "fireSparking";
                    p1["type"]    = "uint8";
                    p1["min"]     = 0;
                    p1["max"]     = 255;
                    p1["default"] = segments[0]->fireSparking;

                    JsonObject p2 = params.createNestedObject();
                    p2["name"]    = "fireCooling";
                    p2["type"]    = "uint8";
                    p2["min"]     = 0;
                    p2["max"]     = 255;
                    p2["default"] = segments[0]->fireCooling;
                    break;
                }
                case PixelStrip::Segment::SegmentEffect::ColoredFire:
                {
                    JsonObject p1 = params.createNestedObject();
                    p1["name"]    = "fireColor1";
                    p1["type"]    = "uint32";
                    p1["default"] = segments[0]->fireColor1;

                    JsonObject p2 = params.createNestedObject();
                    p2["name"]    = "fireColor2";
                    p2["type"]    = "uint32";
                    p2["default"] = segments[0]->fireColor2;

                    JsonObject p3 = params.createNestedObject();
                    p3["name"]    = "fireColor3";
                    p3["type"]    = "uint32";
                    p3["default"] = segments[0]->fireColor3;
                    break;
                }
                case PixelStrip::Segment::SegmentEffect::KineticRipple:
                {
                    JsonObject p1 = params.createNestedObject();
                    p1["name"]    = "rippleWidth";
                    p1["type"]    = "int";
                    p1["min"]     = 1;
                    p1["max"]     = 10;
                    p1["default"] = segments[0]->rippleWidth;
                    ...
                    p2["min"]     = 0.0;
                    p2["max"]     = 1.0;
                    p2["default"] = segments[0]->rippleSpeed;
                    break;
                }
                default:
                    // no extra params
        ...
        Serial.println(out);
        if (connectedCentral)
            cmdCharacteristic.writeValue(out.c_str());
    }
    else
    {
