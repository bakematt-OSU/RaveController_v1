...
inline EffectType effectFromString(const String& str) {
    #define CASE_ENTRY(name, ns) if (str.equalsIgnoreCase(#name)) return EffectType::name;
...
inline void applyEffectToSegment(PixelStrip::Segment* seg, EffectType effect) {
    ...
    switch (effect) {
        #define EFFECT_CASE(name, ns) case EffectType::name: ns::start(seg, color, color); break;
